  ...
  <div class="flex flex-col md:flex-row">
    <div id="sidebar" class="bg-white w-56 h-screen shadow-2xl p-6 hidden md:block fixed top-0 left-0">
      <ul class="space-y-4">
        ...
        <li><button onclick="window.location.href='../admin_dashboard.php'"
            class="w-full text-left text-gray-700 hover:text-blue-600 font-semibold">Dashboard</button></li>

        <li><button onclick="window.location.href='adminCreateLicense.php'"
            class="w-full text-left text-gray-700 hover:text-blue-600 font-semibold">Create License</button></li>

        <li><button onclick="window.location.href='adminSearchLicense.php'"
            class="w-full text-left text-gray-700 hover:text-blue-600 font-semibold">Search & Edit License</button></li>

        <li><button onclick="window.location.href='admin_settings.php'"
    ...
    <div class="bg-white p-8 rounded-2xl shadow-xl max-w-4xl mx-auto w-full h-[85vh] overflow-y-auto">
      ...
      <form action="../../_modules/Controller.php" method="POST" class="forms space-y-6">
        ...
        <div>
          ...
          <input type="text" name="license-number" id="license-number" value="<?= htmlspecialchars($licenseNumber) ?>"
            <?= $licenseNumber ? "readonly" : "" ?> class="w-full p-3 border rounded-lg">
        </div>
        ...
        <div>
          ...
          <div class="flex flex-wrap gap-4 mt-1">
            <?php foreach (["REGISTERED", "UNREGISTERED", "EXPIRED", "REVOKED"] as $s): ?>
        ...
        <div>
          ...
          <div class="flex flex-wrap gap-4 mt-1">
            <?php foreach (["PROFESSIONAL", "NON-PROFESSIONAL", "STUDENT PERMIT"] as $t): ?>
              <label class="flex items-center gap-2">
        ...
        <div>
          ...
          <div class="flex gap-4 mt-1">
            <label class="flex items-center gap-2"><input type="radio" name="expiry-option" value="5"> 5 Years</label>
            <label class="flex items-center gap-2"><input type="radio" name="expiry-option" value="10"> 10 Years</label>
          </div>
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
          <input name="first-name" id="first-name" placeholder="First Name" class="w-full p-3 border rounded-lg">
          <input name="middle-name" id="middle-name" placeholder="Middle Name" class="w-full p-3 border rounded-lg">
          <input name="last-name" id="last-name" placeholder="Last Name" class="w-full p-3 border rounded-lg">
        </div>
        ...
        <input name="address" id="address" placeholder="Address" class="w-full p-3 border rounded-lg">
        <input name="nationality" id="nationality" placeholder="Nationality" class="w-full p-3 border rounded-lg">

        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
          <input name="height" id="Height" placeholder="Height (cm)" class="w-full p-3 border rounded-lg">
          <input name="weight" id="weight" placeholder="Weight (kg)" class="w-full p-3 border rounded-lg">
          <input name="eye-color" id="eye-color" placeholder="Eye Color" class="w-full p-3 border rounded-lg">
        </div>

        <input name="blood-type" id="blood-type" placeholder="Blood Type" class="w-full p-3 border rounded-lg">
